%THINGS TO DO:
%1) Calculate the mean and SD of relative phase for 1 subject
...
load('BC_037_IP.mat'); %CHANGE HERE (with mat file you're interested in)
trials = BC_037_IP; %CHANGE HERE (with mat file you're interested in)
%% Create a folder to store data
%Make folder for results
cd(data_home);
datS01 = 'S01_BC_Data/InPhase'; %THIS WILL ALWAYS BE IN-PHASE
mkdir(datS01);
addpath(genpath(datS01));

%Open summary file for output
dat_summ_path = [data_home '/' datS01];
dat_summ = [dat_summ_path '/BC_data/InPhase]'];  %CHANGE HERE (AntiPhase or InPhase)

%% Iterating through data to save columns we need into a csv

trial_protocol = []; %There are 16 Trial Protocols (TPs) in subject 2-8 and 18 Trial Protocols (TPs) for subject 10-35
...
for k = 1:length(trials) %There are 80 trials total from subject 2-9 and 90 trials total for subjects 10-35
    ...
    filename = strcat('S01_BC_Data/InPhase/Data_IP_037_Trial_',  int2str(k), '_TP_', num2str(TP, '%.f'), '.csv'); %saving the table as a csv with the filename you specify
